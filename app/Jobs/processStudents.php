...
class processStudents extends Job implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $alumno->ID_Municipio      = Municipios::where(["nombre_municipio" => $this->data["student"]->data["municipio"]])->get()[0]->ID_Municipio;
        $alumno->ID_estado         = Estados::where(["nombre_estado" => $this->data["student"]->data["estado"]])->get()[0]->ID_Estado;
        ...
        $alumno->cantidad_hijos    = $this->data["student"]->data["hijos"];
        ...
        $calificaciones = \App\DocCalf::where(["user_sibal" => $alumno->usuario_sibal]);

        if($calificaciones->count() > 0)
        {
            foreach($calificaciones->get() as $expediente)
            {
                //clave_materia debe tener 8 caracteres
                ...
                try{
                    ...
                    $alumno->Expediente->ID_Materia = Materias::where(["clave_materia" => $expediente->matters_id])->firstOrFail()->ID_Materia;
            ...
            Log::info("Alumno creado con ID ".$alumno->ID_alumno." tiene ".$calificaciones->count()." calificaciones.");
        }
        else
        {
            Log::warning("Usuario ".$alumno->ID_alumno." no tiene calificaciones");
        }
    }
}
